...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.studentPayroll.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.student-payrolls.update", [$studentPayroll->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('student') ? 'is-invalid' : '' }}" name="student_id" id="student_id" required>
                    @foreach($students as $id => $entry)
                        <option value="{{ $id }}" {{ (old('student_id') ? old('student_id') : $studentPayroll->student->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('course_amount') ? 'is-invalid' : '' }}" type="text" name="course_amount" id="course_amount" value="{{ old('course_amount', $studentPayroll->course_amount) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paid_amount') ? 'is-invalid' : '' }}" type="number" name="paid_amount" id="paid_amount" value="{{ old('paid_amount', $studentPayroll->paid_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('due_amount') ? 'is-invalid' : '' }}" type="number" name="due_amount" id="due_amount" value="{{ old('due_amount', $studentPayroll->due_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('due_date') ? 'is-invalid' : '' }}" type="text" name="due_date" id="due_date" value="{{ old('due_date', $studentPayroll->due_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('paid_date') ? 'is-invalid' : '' }}" type="text" name="paid_date" id="paid_date" value="{{ old('paid_date', $studentPayroll->paid_date) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{!! old('note', $studentPayroll->note) !!}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
