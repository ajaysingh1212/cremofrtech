    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.teachers.update", [$teacher->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_teacher') ? 'is-invalid' : '' }}" name="select_teacher_id" id="select_teacher_id" required>
                    @foreach($select_teachers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('select_teacher_id') ? old('select_teacher_id') : $teacher->select_teacher->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('courses') ? 'is-invalid' : '' }}" name="courses[]" id="courses" multiple>
                    @foreach($courses as $id => $course)
                        <option value="{{ $id }}" {{ (in_array($id, old('courses', [])) || $teacher->courses->contains($id)) ? 'selected' : '' }}>{{ $course }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_of_joining') ? 'is-invalid' : '' }}" type="text" name="date_of_joining" id="date_of_joining" value="{{ old('date_of_joining', $teacher->date_of_joining) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qualification') ? 'is-invalid' : '' }}" type="text" name="qualification" id="qualification" value="{{ old('qualification', $teacher->qualification) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('salary') ? 'is-invalid' : '' }}" type="number" name="salary" id="salary" value="{{ old('salary', $teacher->salary) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('known_language') ? 'is-invalid' : '' }}" name="known_language" id="known_language">{!! old('known_language', $teacher->known_language) !!}</textarea>
