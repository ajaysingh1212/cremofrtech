    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.students.update", [$student->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_user') ? 'is-invalid' : '' }}" name="select_user_id" id="select_user_id" required>
                    @foreach($select_users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('select_user_id') ? old('select_user_id') : $student->select_user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('refered_by') ? 'is-invalid' : '' }}" name="refered_by_id" id="refered_by_id">
                    @foreach($refered_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('refered_by_id') ? old('refered_by_id') : $student->refered_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_courses') ? 'is-invalid' : '' }}" name="select_courses[]" id="select_courses" multiple>
                    @foreach($select_courses as $id => $select_course)
                        <option value="{{ $id }}" {{ (in_array($id, old('select_courses', [])) || $student->select_courses->contains($id)) ? 'selected' : '' }}>{{ $select_course }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $student->phone_number) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Student::GENDER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('gender') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="gender_{{ $key }}" name="gender" value="{{ $key }}" {{ old('gender', $student->gender) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('parmanet_address') ? 'is-invalid' : '' }}" name="parmanet_address" id="parmanet_address">{!! old('parmanet_address', $student->parmanet_address) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('present_address') ? 'is-invalid' : '' }}" type="text" name="present_address" id="present_address" value="{{ old('present_address', $student->present_address) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_of_joing') ? 'is-invalid' : '' }}" type="text" name="date_of_joing" id="date_of_joing" value="{{ old('date_of_joing', $student->date_of_joing) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('course_fee') ? 'is-invalid' : '' }}" type="number" name="course_fee" id="course_fee" value="{{ old('course_fee', $student->course_fee) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('discount_type') ? 'is-invalid' : '' }}" name="discount_type" id="discount_type">
                    ...
                    @foreach(App\Models\Student::DISCOUNT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('discount_type', $student->discount_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number" name="discount" id="discount" value="{{ old('discount', $student->discount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('due_amount') ? 'is-invalid' : '' }}" type="text" name="due_amount" id="due_amount" value="{{ old('due_amount', $student->due_amount) }}">
